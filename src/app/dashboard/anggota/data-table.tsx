...
export default function AnggotaDataTable() {
  ...
  const selectionCol = useMemo<ColumnDef<Anggota>>(
    () => ({
      id: "select",
      header: ({ table }) => (
        <Checkbox
  ...
  const columns = useMemo<ColumnDef<Anggota>[]>(() => [selectionCol, ...anggotaColumns], [selectionCol]);
    ...
    <div className="space-y-4">
      <Card className="border">
        <CardContent className="pt-6">
          ...
          {deletingId !== null && <div className="mt-2 text-xs text-muted-foreground">Menghapus ID: {deletingId}...</div>}
